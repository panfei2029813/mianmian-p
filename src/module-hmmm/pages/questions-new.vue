      ...
      <el-card shadow="never">
        ...
        >
          ...
          <el-form-item label="目录 :" prop="catalogID">
            <el-select
              v-model="qsNewForm.catalogID"
              @focus="loadDirectorysList"
              no-data-text="无数据，请确认选择的学科"
              clearable
              placeholder="请选择"
            >
              <el-option
          ...
          <el-form-item label="企业 :" prop="enterpriseID">
            <el-select
              v-model="qsNewForm.enterpriseID"
              @focus="loadCompanysList"
              clearable
              placeholder="请选择"
            >
              <el-option
          ...
          <el-row>
            ...
            <el-form-item
              label="城市 :"
              prop="province"
              class="el-form-item-province"
            >
              <el-select
                v-model="qsNewForm.province"
            ...
            <el-form-item
              prop="city"
              class="el-form-item-province el-form-item-city"
            >
              <el-select
                v-model="qsNewForm.city"
                @focus="loadAreaList"
                no-data-text="无数据，请确认选择的城市"
                clearable
                class="city-select"
                placeholder="请选择"
              >
          ...
          <el-form-item label="题型 :" prop="questionType">
            <el-radio-group
              v-model="qsNewForm.questionType"
          ...
          <el-form-item v-if="isShowQuestionType" label="选项 :">
            ...
            <div v-if="isShowChecks === 1">
              <el-row
                ...
                v-for="(item, index) in qsNewForm.options"
                :key="index"
              >
                <el-col class="question-type-col" :span="0.5">
                  <el-radio
                    v-model="radio"
                    :label="item.code"
                    @change="changeRadio"
          ...
          <!-- 提交表单 -->
          <el-form-item>
            <el-button type="primary" @click="submitForm('qsNewRef')"
              >确认提交</el-button
            >
...
export default {
  ...
  props: {},
  data() {
    const videoURLPass = (rule, value, callback) => {
      // 试试验证URL格式自定义校验
    ...
    return {
      ...
      qsNewRules: {
        ...
        videoURL: [{ validator: videoURLPass, trigger: 'blur' }],
  ...
  methods: {
    ...
    changeQuestionType(cback) {
      // 题型值改变时触发，控制选项的变化 cb回调选择的值
    ...
    addSeleAndQuest() {
      const codeStr = this.qsNewForm.options[this.qsNewForm.options.length - 1]
        .code
    ...
    async loadTagsList() {
      ...
      try {
        const { data } = await getTagsList({
          subjectID: this.qsNewForm.subjectID
        })
